...
import { QueueService, JobType, JobPriority } from '../../queue/services/queue.service';
...
export class CronService {
  ...
  async scheduleParisFetch(): Promise<void> {
    ...
    try {
      ...
      if (this.isCircuitOpen()) {
        ...
        this.updateJobStats(jobName, startTime, false, 'Circuit breaker is OPEN');
        return;
      }
      ...
      const queueHealth = await this.queueHealthService.getQueueHealth('air-quality');
    ...
    try {
      ...
      for (const location of trackedLocations) {
        try {
          ...
          const dailyReport = await this.analyticsService.generateDailyReport(city, country);
          ...
          if (dailyReport.averageAQI > 150) {
            await this.notificationsQueue.add('daily-aqi-alert', {
              city,
              country,
          ...
          this.logger.error(`Error processing location ${location}:`, locationError);
        }
      }
  ...
  async weeklyCleanup(): Promise<void> {
    ...
    try {
      ...
      const cleanupPromises = [
        this.queueService.cleanCompletedJobs('air-quality', 7 * 24 * 60 * 60 * 1000),
        this.queueService.cleanCompletedJobs('analytics', 7 * 24 * 60 * 60 * 1000),
        this.queueService.cleanCompletedJobs('notifications', 7 * 24 * 60 * 60 * 1000),
      ];
      ...
      cleanupResults.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          this.logger.error(`Cleanup failed for queue ${index}:`, result.reason);
        }
      });
      ...
      this.logger.log(`Weekly cleanup completed. Total jobs cleaned: ${totalCleaned}`);
      this.updateJobStats(jobName, startTime, true);
  ...
  async performHealthCheck(): Promise<void> {
    ...
    try {
      ...
      const unhealthyQueues = queueHealths.filter(health => health.healthScore < 0.5);
      
      if (unhealthyQueues.length > 0) {
        this.logger.warn(`Unhealthy queues detected: ${unhealthyQueues.map(q => q.queueName).join(', ')}`);
        
        // Add alert notification
        await this.notificationsQueue.add('system-health-alert', {
  ...
  private updateJobStats(jobName: string, startTime: number, success: boolean, error?: string): void {
    const stats = this.jobStats.get(jobName);
    if (!stats) return;
  ...
  private async clearOldCacheEntries(): Promise<void> {
    ...
    this.logger.log('Cache cleanup completed (implementation depends on cache store)');
  }
  ...
  private async cleanupOldJobStats(): Promise<void> {
    ...
    for (const [jobName, stats] of this.jobStats) {
      if (stats.lastExecution && Date.now() - stats.lastExecution.getTime() > 30 * 24 * 60 * 60 * 1000) {
