...
import {
  ...
  QueueHealthMetrics,
  QueueBottleneck,
} from '../../queue/services/queue-health.service';
...
export class CronController {
  ...
  @ApiResponse({
    status: 200,
    description: 'CRON job statistics retrieved successfully',
  })
  ...
  async toggleCronJob(
    ...
    this.logger.log(
      `Toggling CRON job ${jobName} to ${toggleDto.enabled ? 'enabled' : 'disabled'}`
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Queue bottlenecks detected successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Queue recommendations retrieved successfully',
  })
  async getQueueRecommendations(
    @Param('queueName') queueName: string
  ): Promise<string[]> {
  ...
  @ApiResponse({
    status: 200,
    description: 'System status overview retrieved successfully',
  })
  async getSystemOverview(): Promise<{
    ...
    const [cronJobs, queueHealth, circuitBreaker, bottlenecks] =
      await Promise.all([
    ...
    const avgHealthScore =
      queueHealth.reduce((sum, q) => sum + q.healthScore, 0) /
      queueHealth.length;
  ...
  @ApiResponse({
    status: 200,
    description: 'System cleanup triggered successfully',
  })
