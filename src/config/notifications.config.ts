...
export default registerAs('notifications', (): NotificationsConfig => ({
  ...
  alerts: {
    thresholds: {
      consecutive_api_failures: parseInt(process.env.ALERT_API_FAILURES || '5'),
      high_pollution_aqi: parseInt(process.env.ALERT_HIGH_POLLUTION_AQI || '150'),
      extreme_pollution_aqi: parseInt(process.env.ALERT_EXTREME_POLLUTION_AQI || '200'),
      ...
      system_error_rate: parseFloat(process.env.ALERT_SYSTEM_ERROR_RATE || '0.1'),
      storage_usage_threshold: parseFloat(process.env.ALERT_STORAGE_USAGE || '0.8'),
      email_rate_limit: parseInt(process.env.ALERT_EMAIL_RATE_LIMIT || '50'),
    },
    recipients: {
      adminRecipients: process.env.ALERT_ADMIN_RECIPIENTS?.split(',') || ['admin@aqi-api.com'],
      pollutionRecipients: process.env.ALERT_POLLUTION_RECIPIENTS?.split(',') || ['admin@aqi-api.com'],
      systemRecipients: process.env.ALERT_SYSTEM_RECIPIENTS?.split(',') || ['admin@aqi-api.com'],
      escalationRecipients: process.env.ALERT_ESCALATION_RECIPIENTS?.split(',') || ['admin@aqi-api.com'],
    },
    throttling: {
      defaultThrottleMinutes: parseInt(process.env.ALERT_THROTTLE_MINUTES || '30'),
      escalationMinutes: parseInt(process.env.ALERT_ESCALATION_MINUTES || '60'),
